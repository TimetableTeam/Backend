...
async function validateDraft(versionId, resolvedConflictIds = []) {
  ...
  for (const alloc of allocations) {
    ...
    // {
    //   result: { feasible, conflicts, ... },
    //   resolved: { ... }
    // }
    const { result: conflictResult } =
      await allocationService.checkConflicts({
  ...
  const scheduledSectionIds = new Set(
    allocations.map(a => a.section_id)
  );
...
async function submitForReview(versionId, actorId, resolvedConflictIds = []) {
  ...
  if (!validation.valid) {
    throw ApiError.conflict(
      'Cannot submit for review: hard conflicts exist',
      {
 ...
 */
async function updateDraftAllocation(
  versionId,
  allocationId,
  updates,
  actor
  ...
  if (!version) {
    throw ApiError.notFound('Schedule version not found.');
  }

  if (
    version.state !== 'DRAFT' &&
    version.state !== 'SUBMITTED_FOR_REVIEW'
  ) {
    throw ApiError.badRequest(
      'Only DRAFT or SUBMITTED_FOR_REVIEW versions can be edited.'
    );
  }
...
/**
 * Publish draft (Admin workflow)
 * Archives previous published version, then publishes this one.
 */
async function publishDraft(versionId, actorId) {

  // Final Tanseek workflow:
  // A Scheduler must submit the draft,
  // then Admin publishes it.
  const validation = await validateDraft(versionId);

  if (!validation.valid) {
    throw ApiError.conflict(
      'Cannot publish: hard conflicts exist',
      {
        violations: validation.conflicts,
      }
    );
  ...
  const review = await require('../db/pool').query(
    `SELECT 1
     FROM audit_events
     WHERE entity_type = 'schedule_versions'
       AND entity_id = $1
  ...
  if (!review.rows.length) {
    throw ApiError.conflict(
      'The Scheduler must submit this draft for Admin review before publishing.'
    );
  }
  ...
  return withTransaction(async (client) => {
    ...
    if (
      v.state !== 'DRAFT' &&
      v.state !== 'SUBMITTED_FOR_REVIEW'
    ) {
      throw ApiError.badRequest(
        'Only DRAFT or SUBMITTED_FOR_REVIEW versions can be published.'
      );
    }
