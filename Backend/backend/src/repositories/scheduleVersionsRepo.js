...
async function findById(id) {
  const res = await query(`SELECT * FROM schedule_versions WHERE id = $1`, [id]);
  return res.rows[0] || null;
}

async function findPublished(termId) {
  const res = await query(`SELECT * FROM schedule_versions WHERE term_id = $1 AND state = 'PUBLISHED'`, [termId]);
  return res.rows[0] || null;
}

async function nextVersionNumber(termId) {
  const res = await query(`SELECT COALESCE(MAX(version_number), 0) + 1 AS next FROM schedule_versions WHERE term_id = $1`, [termId]);
  return res.rows[0].next;
}

async function createDraft({ termId, name, createdBy }, client) {
  const runner = client || { query };
  const versionNumber = await nextVersionNumber(termId);
